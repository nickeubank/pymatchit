...
def test_global_caliper(lalonde_data):
    ...
    m_caliper = MatchIt(
        lalonde_data, method="nearest", distance="glm", caliper=0.01
    )  # Sehr streng
...
def test_string_dtype_parsing(lalonde_data):
    ...
    try:
        ...
        pytest.fail(
            "TypeError raised! Der Pandas 2.0 StringDtype Fix funktioniert nicht."
        )
...
def test_ecdf_plot_categorical_error(lalonde_data):
    ...
    with pytest.raises(
        TypeError, match="mathematically defined for continuous/numeric variables only"
    ):
