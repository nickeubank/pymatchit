...
from .diagnostics import compute_sample_size_table, create_summary_table
...
from .plotting import ecdf_plot, jitter_plot, love_plot, propensity_plot, qq_plot
    ...
    def __init__(
        ...
        Args:
            ...
            method (str): Matching algorithm ('nearest', 'optimal', 'exact',
                ...
                Default 'nearest'.
            distance (str or array): Distance metric or pre-computed propensity scores.
                Strings: 'glm', 'cbps', 'mahalanobis', 'randomforest',
                'decisiontree', 'neuralnet', 'gbm', 'adaboost', 'lasso',
                'ridge', 'elasticnet'. Arrays: numpy array or pandas Series of
                pre-computed propensity scores (one per row of `data`).
                Default 'glm'.
                Ignored for methods of exact or cem.
            link (str): Link function for distance estimation: 'logit', 'probit'
                (GLM only), 'linear.logit', or 'linear'. With 'logit' or
                'linear.logit', matching is done on the linear (logit-scale)
                score rather than the raw probability. Default 'logit'.
                Ignored for methods of exact or cem.
            replace (bool): Whether control units can be reused as matches for
                multiple treated units (nearest, exact, and genetic matching).
                Default False.
            caliper (float or dict): A float acting as a global threshold on the
                distance measure (in standard deviations), or a dict for
                covariate-specific limits (e.g. {'distance': 0.1, 'age': 2}).
                Used by nearest, optimal, full, and genetic matching.
                Default None (no caliper).
            ratio (int): Number of control units to match to each treated unit
                (nearest, optimal, exact, and genetic matching). Default 1.
                Ignored for methods of exact or cem.
            estimand (str): Target estimand: 'ATT' (average treatment effect on
                the treated), 'ATE', or 'ATC'. Default 'ATT'.
            subclass (int): Number of subclasses for subclassification
                (method='subclass'). Default 6.
            discard (str): Which units to discard for falling outside the common
                support of the propensity score before matching: 'none',
            ...
            antiexact (str or list): Variable(s) on which matched units must have
                DIFFERENT values. Default None.
            m_order (str): Order in which matches are assigned for nearest
                neighbor matching: 'largest', 'smallest', 'random', or 'data'
                (original row order). Default 'largest'.
            mahvars (list): Variables on which to compute Mahalanobis distance
                within a propensity score caliper. Default None.
            cutpoints (dict): For CEM (method='cem'): per-covariate binning,
                mapping column name to either an integer number of bins or an
                explicit list of cut points (passed to ``pd.cut``).
                Covariates not listed that have a numeric dtype (`int`, `float`,
                `uint`, or `complex`) are binned using Sturges' rule. `bool` and
                `categorical` dtypes are not coarsened.
                Default None.
            tols (dict): Covariate-specific balance tolerances for cardinality
                matching (method='cardinality'). Default None.
            std_tols (float): Default standardized mean difference tolerance for
                cardinality matching, applied to covariates not in `tols`.
                Default 0.1.
            pop_size (int): Population size for genetic matching optimization
                (method='genetic'). Default 100.
            ...
            min_controls_per_subclass (int): Minimum number of control units per
                subclass in full matching (method='full'). Default 1.
            ...
            distance_options (dict): Options passed to the distance estimation
                ...
                Default None.
            random_state (int): Seed for reproducibility of stochastic steps
                (e.g. m_order='random', genetic matching, ML-based distance
        ...
        if format == "long":
            ...
            for t_idx, c_indices in self.matched_indices.items():
                for c_idx in c_indices:
                    rows.append({"treated_index": t_idx, "control_index": c_idx})
